...
app.post('/', async (c) => {
  ...
  try {
    ...
    console.log('Sending email via Mailjet. Request object:', request);
    const result = await request;
    console.log('Mailjet API Response (full result object):', result);

    if (result && result.body) {
      console.log('Mailjet API Response Body:', result.body);
    } else {
      console.log('Mailjet API Response: Result or result.body is undefined/null.', result);
    }
