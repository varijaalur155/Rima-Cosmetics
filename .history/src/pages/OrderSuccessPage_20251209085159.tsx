      ...
      const sendOrderConfirmationEmail = async () => {
        try {
          const templateParams = {
            ...
            items_html: order.items.map((item: { product_id: string; name: string; quantity: number; price: number }) => `<li>${item.name} x ${item.quantity} (₹${item.price * item.quantity})</li>`).join(''),
          };
