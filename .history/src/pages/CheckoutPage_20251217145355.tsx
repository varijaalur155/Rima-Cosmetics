...
export function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailOrderId = `ORD-${Date.now()}`;
      ...
      if (paymentMethod === 'google-pay') {
        globalThis.location.href = `upi://pay?pa=mkmounica4@oksbi&pn=Rima Organic Cosmetics&am=${totalPrice}&cu=INR&tn=Order ${orderId}`;
      } else if (paymentMethod === 'phonepe') {
        globalThis.location.href = `upi://pay?pa=8939996640@ybl&pn=Rima Organic Cosmetics&am=${totalPrice}&cu=INR&tn=Order ${orderId}`;
      } else if (paymentMethod === 'whatsapp-pay') {
        globalThis.location.href = `https://wa.me/918939996640?text=Payment for order ${orderId}. Amount: ₹${totalPrice}`;
      }
                    ...
                    <div className="mt-4">
                      {paymentMethod === 'google-pay' && (
                        <p className="text-green-700 text-sm">
                          Pay using Google Pay UPI ID: <strong>mkmounica4@oksbi</strong>. After payment, send the payment screenshot to the business owner via WhatsApp or email.
                        </p>
                      )}
                      {paymentMethod === 'phonepe' && (
                        <p className="text-green-700 text-sm">
                          Pay using PhonePe UPI ID: <strong>8939996640@ybl</strong>. After payment, send the payment screenshot to the business owner via WhatsApp or email.
                        </p>
                      )}
                      {paymentMethod === 'whatsapp-pay' && (
                        <p className="text-green-700 text-sm">
                          After placing your order, you'll be contacted via WhatsApp to complete the payment. After payment, send the payment screenshot to the business owner.
                        </p>
                      )}
                    </div>
                  </div>
