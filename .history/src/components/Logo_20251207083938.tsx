...
export function Logo({ className = "w-10 h-10 object-contain" }: { className?: string }) {
  ...
  // If you place your provided `BusinessLogo.svg` into the `public/` folder
  // (path: `/BusinessLogo.svg`) the <img> below will be used. If the file
  // is missing or fails to load, we fall back to the inline SVG defined
  // here so the site still shows a logo.
  if (!imgError) {
    return (
      // `object-contain` keeps the logo within the given box and aligns nicely
      <img
